...
rich_click ships ``py.typed``, but it re-exports click's ``option``,
``argument`` and ``version_option`` decorators, whose ``type: ParamType[Unknown]``
parameter makes pyright (strict mode) report ``reportUnknownMemberType`` at every
call site. Wrapping the affected decorators here behind explicit, fully-known
signatures keeps the CLI layer strict-clean without disabling the rule. This
module is the single boundary that touches the untyped surface, so the only
``# pyright: ignore`` for this third-party gap lives here.

Other click members (``command``, ``group``, ``echo``, ``Context``, ``Path``,
``Choice``, ``pass_context`` …) type cleanly and are still used directly as
``click.X`` at call sites.
"""
...
# collected into a list (shared-options pattern).
