...
Thanks for helping improve **lib_registry**. The sections below summarise the day-to-day workflow, highlight the repository automation, and list the checks that must pass before a change is merged.
...
- CLI tests that create registry keys must use the `temp_key` pytest fixture (or similar) for guaranteed teardown cleanup. Never rely on assertions passing before cleanup code runs.
- CLI tests that create temporary files must use the `temp_file` fixture.
- Whenever you add a Registry method, CLI command, or change metadata, update the relevant tests in `tests/` and doctests in `registry.py` so the specification remains complete.
